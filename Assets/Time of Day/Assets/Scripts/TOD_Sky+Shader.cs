...
public partial class TOD_Sky : MonoBehaviour
{
	...
	{
		return MoonHaloColor * Mathf.Pow(Mathf.Max(0, Vector3.Dot(dir, LocalMoonDirection)), 1f / Moon.HaloSize);
	}
	...
	private Color ShaderScatteringColor(Vector3 dir, bool directLight = true)
	{
		...
		// Ground factor
		float ground = Mathf.SmoothStep(0.0f, 1.25f, -dir.y);
		...
		// Lerp to fog color
		resultColor_r = Mathf.Lerp(resultColor_r, CloudColor.r, Atmosphere.Fogginess);
		resultColor_g = Mathf.Lerp(resultColor_g, CloudColor.g, Atmosphere.Fogginess);
		resultColor_b = Mathf.Lerp(resultColor_b, CloudColor.b, Atmosphere.Fogginess);

		// Lerp to ground color
		resultColor_r = Mathf.Lerp(resultColor_r, AmbientColor.r, ground);
		resultColor_g = Mathf.Lerp(resultColor_g, AmbientColor.g, ground);
		resultColor_b = Mathf.Lerp(resultColor_b, AmbientColor.b, ground);
