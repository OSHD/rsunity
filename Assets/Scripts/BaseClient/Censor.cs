...
public class Censor {
    ...
    private static void method493(Stream stream, char[][] ac, byte[][][] abyte0)
    {
        for(int j = 0; j < ac.Length; j++)
        {
            ...
            ac[j] = ac1;
            byte[][] abyte1 = new byte[stream.readUnsignedByte()][];
			for(int i = 0; i < abyte1.GetLength(0); i++)
				abyte1[i] = new byte[2];
    ...
    private static bool method496(char c)
    {
    ...
    public static String doCensor(String s)
    {
        ...
        for(int j = 0; j < exceptions.Length; j++)
        {
            for(int k = -1; (k = s2.IndexOf(exceptions[j], k + 1)) != -1;)
            {
                ...
                Array.Copy(ac1, 0, ac, k, ac1.Length);
    ...
    private static bool method510(byte byte0, byte[][] abyte0, byte byte2)
    {
        int i = 0;
    ...
    private static bool method517(char c)
    {
        return !isLetter(c) && !isDigit(c);
    }

    private static bool method518(char c)
    {
    ...
    private static bool method523(char[] ac)
    {
        bool flag = true;
