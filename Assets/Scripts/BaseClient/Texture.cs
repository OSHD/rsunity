...
public class Texture : DrawingArea {
		...
		public static void method367()
		{
			if(anIntArrayArray1478 == null)
			{
				...
				if(lowMem)
					anIntArrayArray1478 = NetDrawingTools.CreateDoubleIntArray(anInt1477, 16384); //new int[anInt1477][16384];
				else
					anIntArrayArray1478 = NetDrawingTools.CreateDoubleIntArray(anInt1477, 0x10000); //new int[anInt1477][0x10000];
				for(int k = 0; k < 50; k++)
		...
		public static void method368(StreamLoader streamLoader)
		{
			...
			for(int j = 0; j < 50; j++)
				try
			{
				aBackgroundArray1474s[j] = new Background(streamLoader, j.ToString(), 0);
...
//			catch(Exception ex)
//			{
//				UnityEngine.Debug.Log ("method371Error: " + ex.Message);
//				return new int[0];
//			}
		...
		{
			d += Random.NextDouble() * 0.029999999999999999D - 0.014999999999999999D;
			int j = 0;
			for(int k = 0; k < 512; k++)
			{
		...
		public static void drawShadedTriangle(int i, int j, int k, int l, int i1, int j1, int k1, int l1, 
		                                      int i2,  float z_a, float z_b, float z_c)
		{
		...
		public static void drawFlatTriangle(int i, int j, int k, int l, int i1, int j1, int k1, float z_a, float z_b, float z_c)
		{
			if (z_a < 0 || z_b < 0 || z_c < 0) {
				return;
			}
		...
		public static void drawTexturedTriangle(int i, int j, int k, int l, int i1, int j1, int k1, int l1, 
		                             int i2, int j2, int k2, int l2, int i3, int j3, int k3, 
		                                        int l3, int i4, int j4, int k4, float z_a, float z_b, float z_c)
		{
			if (z_a < 0 || z_b < 0 || z_c < 0) {
				return;
			}
		                              ...
		                              int k1, int l1, int i2, int j2, int k2, int l2, int i3)
		{
			...
			if(lowMem)
			{
				...
				if(aBoolean1463)
				{
					while(k3-- > 0) 
					{
						ai[k++] = (int)((uint)ai1[(j & 0xfc0) + (i >> 6)] >> i8);
						i += i7;
						j += k7;
						ai[k++] = (int)((uint)ai1[(j & 0xfc0) + (i >> 6)] >> i8);
						i += i7;
				...
				{
					int k8;
					if((k8 = (int)((uint)ai1[(j & 0xfc0) + (i >> 6)] >> i8)) != 0)
						ai[k] = k8;
					k++;
					i += i7;
					j += k7;
				...
				{
					int l8;
					if((int)((uint)(l8 = ai1[(j & 0xfc0) + (i >> 6)] >> i8)) != 0)
						ai[k] = l8;
					k++;
					i += i7;
					j += k7;
		...
		public static bool notTextured = true;
